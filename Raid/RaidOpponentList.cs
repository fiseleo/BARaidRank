...
namespace mxdat
{
    ...
    {
        public static bool shouldContinue = false; // 新旗標
        public static int savedRankValue = 1;         // 暫存執行到暫停前的 rank 值
        ...
        private static void ExecuteMainLogic(string[] args, DateTime seasonEndData, DateTime settlementEndDate, int rankValue)
        {
            ...
            // ※ 不再使用 JSON 檔案資料夾，因為我們改用 SQLite 儲存
        ...
        {
            // 資料庫檔案路徑
            string dbPath = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "raid_opponent_list.db");
            ...
            // 取得 OpponentUserDBs 陣列的筆數（若有此欄位）
            int opponentCount = 0;
            ...
            ";
                ...
                using (var command = connection.CreateCommand())
                {
                    command.CommandText =
                    @"
            INSERT INTO OpponentList (OuterProtocol, InnerProtocol, Rank, ServerTimeTicks, ServerNotification, OpponentCount, JsonData, InsertedAt)
            VALUES ($outerProtocol, $innerProtocol, $rank, $serverTimeTicks, $serverNotification, $opponentCount, $jsonData, $insertedAt);
            ...
            ";
                ...
                // 如果有 OpponentUserDBs 陣列，逐筆解析並存入 OpponentDetail 資料表
                if (opponentArray != null)
                {
                    int opponentIndex = 1;
                    foreach (var opponentToken in opponentArray)
                    {
                        ...
                        // 從每筆對手資料中解析需要的欄位（這裡取自最外層對手物件）
                        int oppAccountId = opponent["AccountId"]?.Value<int>() ?? 0;
                        int oppRepresentCharUniqueId = opponent["RepresentCharacterUniqueId"]?.Value<int>() ?? 0;
                    ...
                    VALUES 
                        ($mainRecordId, $opponentIndex, $oppAccountId, $oppRepresentCharacterUniqueId, $oppLevel, $oppNickname, $oppTier, $oppRank, $oppBestRankingPoint, $oppBestRankingPointDetail, $oppEmblemUniqueId, $insertedAt);
                    ";
                            ...
                            command.Parameters.AddWithValue("$oppRepresentCharacterUniqueId", oppRepresentCharUniqueId);
        ...
        /// <summary>
        /// 上傳 JSON 至伺服器（保持原有邏輯不變，但 serverUrl 從 ip.txt 讀取）
        /// </summary>
